...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createHospital() {
    ...
    const body = { ...req.body };
    if (body?.screening_question?.length > 0) {
      const create_screening_question = body.screening_question;
      body.screening_question = {
        create: create_screening_question,
      };
    } else {
      delete body.screening_question;
    }
